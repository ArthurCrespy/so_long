...
/*   Updated: 2023/02/23 12:57:32 by acrespy          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_black_hole(t_vars *vars, int direction)
{
	...
	i_col = vars->player.y / 64;
	i_row = vars->player.x / 64;
	if (direction == 1 && vars->map.map[i_col - 1][i_row] == 'X')
		mlx_exit(vars, "You loose!\n", 0);
	if (direction == 2 && vars->map.map[i_col + 1][i_row] == 'X')
		mlx_exit(vars, "You loose!\n", 0);
	if (direction == 3 && vars->map.map[i_col][i_row - 1] == 'X')
		mlx_exit(vars, "You loose!\n", 0);
	if (direction == 4 && vars->map.map[i_col][i_row + 1] == 'X')
		mlx_exit(vars, "You loose!\n", 0);
	return (0);
}
